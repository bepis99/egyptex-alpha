...
const PrimaryBackgroundContainer = tw(Container)`-mx-8 px-8 bg-yellow-900 text-gray-100`;
...
const QuoteIcon = tw(QuoteIconBase)`absolute opacity-15 top-0 left-0 transform -translate-y-2 -translate-x-1/2 sm:-translate-x-full w-10 fill-current text-yellow-700`
...
const ControlButton = styled.button`
  ${tw`text-gray-600 hover:text-yellow-700 focus:outline-none transition-colors duration-300 ml-4 first:ml-0 sm:first:ml-4 mt-4 sm:mt-0`}
  .icon {
    ${tw`fill-current w-6`}
  }
`;
