...
export default ({
  subheading = "",
  heading = <>Medical <span tw="text-yellow-700">Tourism.</span></>,
  description = "",

}) => {
  const [MTourismInfoList, SetMTourismInfo]=useState([]);
  useEffect(()=>{
    axios.get(`${BASE_API_URL}/api/mtourisminfo`).then((response)=>{
      SetMTourismInfo(response.data);})
    })
  ...
  <AnimationRevealPage >
    ...
    <Container>
      <Content>
        ...
        <ThreeColumn>
          {MTourismInfoList.map((val, index) => (
            <Column key={index}>
              <Card>
                <Image imageSrc={val.mtourism_imgsrc} />
                <Details>
                  <MetaContainer>
                    <Meta>
                      <GlobeIcon />
                      <div>{val.mtourism_location}</div>
                    </Meta>
                  </MetaContainer>
                  <Title>{val.mtourism_title}</Title>
                  <Description>{val.mtourism_descritption}</Description>
