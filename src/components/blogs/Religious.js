...
export default ({
  subheading = "",
  heading = <>Religious <span tw="text-yellow-700">Tourism.</span></>,
  description = "",

}) => {
  const [RTourismInfoList, SetRTourismInfo]=useState([]);
  useEffect(()=>{
    axios.get(`${BASE_API_URL}/api/rtourisminfo`).then((response)=>{
      SetRTourismInfo(response.data);})
    })
  ...
  <AnimationRevealPage>
    ...
    <Container>
      <Content>
        ...
        <ThreeColumn>
          {RTourismInfoList.map((val, index) => (
            <Column key={index}>
              <Card>
                <Image imageSrc={val.rtourism_imgsrc} />
                <Details>
                  <MetaContainer>
                    <Meta>
                      <GlobeIcon />
                      <div>{val.rtourism_location}</div>
                    </Meta>
                  </MetaContainer>
                  <Title>{val.rtourism_title}</Title>
                  <Description>{val.rtourism_descritption}</Description>
