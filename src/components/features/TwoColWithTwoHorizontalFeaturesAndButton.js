...
const DecoratorBlob = tw(
  SvgDotPattern
)`w-20 h-20 absolute right-0 bottom-0 transform translate-x-1/2 translate-y-1/2 fill-current text-yellow-700 -z-10`;
...
const FeatureIconContainer = styled.div`
  ${tw`mx-auto inline-block border border-yellow-500 text-yellow-700 text-center rounded p-2 flex-shrink-0`}
  ${props => [
    props.iconRoundedFull && tw`rounded-full`,
    props.iconFilled && tw`border-0 bg-yellow-700 text-gray-100`
  ]}
  svg {
    ${tw`w-5 h-5`}
  }
...
export default ({
  ...
  heading = (
    <>
      Designed & Developed by <span tw="text-yellow-700">Professionals.</span>
    </>
  ),
